...
static esp_err_t root_get_handler(httpd_req_t *req)
{
    static const char page[] =
        ...
        "h2{margin-top:0}"
        ".row{font-size:1.3em;margin:6px 0;padding:6px;border-radius:4px;cursor:pointer;white-space:pre}"
        ".row:active{background:#fff;color:#1a1a2e}"
        ...
        "<script>"
        "const METRICS=["
        "{name:'DH Temp', unit:'C',   ok:d=>d.dht_ok, val:d=>d.dht_temp.toFixed(1),"
        " row:d=>d.dht_temp.toFixed(1)+' C'},"
        "{name:'DH Humi', unit:'%',   ok:d=>d.dht_ok, val:d=>d.dht_humi.toFixed(0),"
        " row:d=>d.dht_humi.toFixed(0)+' %'},"
        "{name:'SCD CO2', unit:'ppm', ok:d=>d.scd_ok, val:d=>String(d.co2),"
        " row:d=>d.co2+' ppm'},"
        "{name:'SCD TEMP',unit:'C',   ok:d=>d.scd_ok, val:d=>d.scd_temp.toFixed(1),"
        " row:d=>d.scd_temp.toFixed(1)+' C'},"
        "{name:'SCD-Humi',unit:'%',   ok:d=>d.scd_ok, val:d=>d.scd_humi.toFixed(0),"
        " row:d=>d.scd_humi.toFixed(0)+' %'},"
        "{name:'BATT',    unit:'%',   ok:d=>d.batt_ok,val:d=>d.batt_pct.toFixed(0),"
        ...
        "    r.textContent=m.name+'  --'; c.appendChild(r);"
        "  });"
        "  rowsBuilt=true;"
        "}"
        "async function tick(){"
