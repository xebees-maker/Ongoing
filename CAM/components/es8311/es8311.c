...
/* 2026-09-26(프로젝트 수정본) — 레지스트리 espressif/es8311 1.0.0은 IDF 6에서 없어진 옛 I2C 드라이버
 * (driver/i2c.h, i2c_master_write_to_device(port,...))를 써서 빌드 불가(IDF 6 지원 버전 없음).
 * BSP가 이미 새 드라이버(i2c_master)로 버스를 열어두므로, 그 버스 핸들에 디바이스를 붙여 씀 */
#define ES8311_I2C_TIMEOUT_MS 1000
#define ES8311_I2C_SCL_HZ     100000   /* BSP 버스 속도(BSP_CAM_I2C_FREQ_HZ)와 같게 */
...
static inline esp_err_t es8311_write_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t data)
{
    ...
    return i2c_master_transmit(es->i2c_dev, write_buf, sizeof(write_buf), ES8311_I2C_TIMEOUT_MS);
}

static inline esp_err_t es8311_read_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t *reg_value)
{
    es8311_dev_t *es = (es8311_dev_t *) dev;
    return i2c_master_transmit_receive(es->i2c_dev, &reg_addr, 1, reg_value, 1, ES8311_I2C_TIMEOUT_MS);
}
...
es8311_handle_t es8311_create(i2c_master_bus_handle_t bus, const uint16_t dev_addr)
{
    ...
    es8311_dev_t *es = (es8311_dev_t *) calloc(1, sizeof(es8311_dev_t));
    if (!es) {
        ESP_LOGE(TAG, "no memory for ES8311 handle");
        return NULL;
    }
    const i2c_device_config_t dev_cfg = {
        .dev_addr_length = I2C_ADDR_BIT_LEN_7,
        .device_address = dev_addr,
        .scl_speed_hz = ES8311_I2C_SCL_HZ,
    };
    esp_err_t err = i2c_master_bus_add_device(bus, &dev_cfg, &es->i2c_dev);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "i2c_master_bus_add_device failed: %s", esp_err_to_name(err));
        free(es);
        return NULL;
    }
    return (es8311_handle_t) es;
}
