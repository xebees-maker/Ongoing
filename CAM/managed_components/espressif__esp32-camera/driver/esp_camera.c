...
/* 2026-07-23: 원래 4000ms — 5MP/QXGA 같은 고해상도 JPEG은 XCLK 20MHz/PCLK 12.5MHz 기준
 * 실측상 4초 안에 못 끝나고 timeout으로 실패하는 걸 확인(project_cam_dvp_corruption_investigation
 * 메모리 참고). 진짜로 손상된 게 아니라 단순히 더 걸리는 것뿐이라 넉넉히 늘림. */
#define FB_GET_TIMEOUT (15000 / portTICK_PERIOD_MS)
