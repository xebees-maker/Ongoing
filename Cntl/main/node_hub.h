 ...
 * 페어링(PAIR_REQUEST/ACK) 처리. 구 Cntl의 node_hub.c에서 페어링/노드테이블 부분만
...
#define NODE_HUB_NODE_TIMEOUT_MS 50000U
 ...
 * 보관(photo_rx.c의 요청 구조체 중 가장 큰 것도 몇 바이트뿐이라 여유있게 잡음).
 * ack_types는 node_request_enqueue()와 동일 계약(호출부의 static 배열 포인터를 그대로
 * 보관 — 비동기로 나중에 읽히므로 임시 스택 배열이면 안 됨) */
#define NODE_HUB_PENDING_ACTION_MAX_LEN   16
#define NODE_HUB_PENDING_ACTION_QUEUE_DEPTH 4

typedef struct {
    uint8_t        req[NODE_HUB_PENDING_ACTION_MAX_LEN];
    size_t         req_len;
...
typedef struct {
     ...
     * 이 플래그로 막음(node_hub_pair()가 다시 호출되기 전까진 PAIR_ACK를 무시) */
    bool            user_unpaired;
     ...
     * node_hub_is_reconnect_stuck()이 "마지막으로 페어링 성공한 지 얼마나 됐는가"를
     * 판단하는 기준 */
     ...
     * (0=대기 중 아님, node_hub_request_pair()가 3으로 세팅) */
     ...
     * photo_rx.c의 각 함수가 "지금 보낼 수 있나"를 따로 확인할 필요가 없어짐(그 판단은
     * 여기 한 곳, WAKE_HELLO 핸들러로 모임). 링버퍼 — 가득 차면 가장 오래된 것부터 버림 */
    node_hub_pending_action_t action_queue[NODE_HUB_PENDING_ACTION_QUEUE_DEPTH];
     ...
     * (node_hub.c의 WAKE_HELLO_SENS 분기 참고) */
 ...
 * 자동으로 풀림(node_hub.c의 GOT_IP 핸들러 참고) */
...
typedef void (*node_hub_sta_test_cb_t)(bool success, void *ctx);
...
} node_hub_sta_test_stage_t;
typedef void (*node_hub_sta_test_stage_cb_t)(node_hub_sta_test_stage_t stage, void *ctx);
...
/* 리스트 아이템 탭 시 사용 — mac은 node_hub_get_nodes()로 얻은 노드의 mac[6].
...
typedef void (*node_hub_event_cb_t)(void);
 ...
 * 반환 — node_hub_get_conn_state()의 WAITING 판정에 씀. 기준 시간은
 * node_hub_node_timeout_ms()와 동일 — 이미 "이 정도 무응답이면 정상 범위를 넘었다"는
...
/* 적응형 반응시간(2026-08-10) — photo_rx.c의 5개 사용자 액션 함수(지금촬영/목록갱신/
...
/* 2026-08-26(사용자 지시) — 사용자 액션을 노드별 대기 큐에 넣음(photo_rx.c의 5개 액션
 * 함수가 호출). 지금 이 노드가 붙어있는지 여기서 확인 안 함 — 다음 WAKE_HELLO의 CASK
 * "할일" 단계에서 node_hub.c가 큐를 확인해 하나씩 꺼내 보냄(중앙집중 판단, 여기저기서
 * 반복 확인할 필요 없음). req/req_len은 그대로 복사되어 보관됨(NODE_HUB_PENDING_ACTION_MAX_LEN
 * 이하만 가능). ack_types는 node_request_enqueue()와 동일 계약(static 배열 필수 — 나중에
 ...
 * Apply 동작 전용. 둘 다 node_request 큐잉 패턴이라 즉시 리턴(UI 안 얼어붙음) — 실제 결과는
 * CAM_CONFIG_ACK 수신으로 옴(node_request가 재시도까지 처리, 실패해도 로그/토스트만).
 *
