 ...
 *
 * recv_cb(ESP-NOW/WiFi 드라이버 태스크)에서 photo_rx_on_recv()로 넘겨받아 처리한다.
 * 검증된 사진(압축 JPEG 원본 그대로)은 file_id별 캐시에 저장되고, UI는 photo_rx_cache_get()
 ...
 * ──────────────────────────────────────────────────────────── */
#define PHOTO_RX_LIST_MAX 64  /* CAM SD엔 최대 500장까지 있을 수 있지만 화면 리스트라 제한 */
...
typedef struct {
    ...
    uint32_t file_size;
} photo_rx_list_view_item_t;

typedef enum {
    PHOTO_RX_LIST_STATE_IDLE = 0,
    PHOTO_RX_LIST_STATE_REQUESTING,
    PHOTO_RX_LIST_STATE_READY,
    PHOTO_RX_LIST_STATE_ERROR,  /* 2026-08-11 — DONE이 이미 아는 개수보다 적게 받은 상태로
                                        조기도착(개수 불일치). get_progress()의 received/expected가
                                        실패 시점 값 그대로 남아있어 팝업이 사유를 표시할 수 있음 */
} photo_rx_list_state_t;

/* 반환값: 이 요청의 세대번호 — photo_rx_list_get_result()에 그대로 넘기면 ack로 소비되지
...
/* 2026-08-30 — photo_rx_list_ack()로 소비되지 않는, generation 기준 결과 확인. generation은
 * photo_rx_list_request()가 돌려준 값. true면 그 요청이 끝났다는 뜻(성공/실패는 *out_ok),
 * false면 아직 진행 중이거나 그 뒤로 새 요청이 없었음. 결과가 나왔으면 photo_rx_list_get_items()로
 * 그대로 목록을 읽으면 됨(버퍼 자체는 ack와 무관하게 안 지워짐) */
bool photo_rx_list_get_result(uint32_t generation, bool *out_ok);

/* 2026-09-04(사용자 설계: "이벤트로 처리해") — 위 photo_rx_set_ready_cb()와 동일 패턴,
...
/* READY일 때 목록을 out에 채워서 개수 반환(최대 PHOTO_RX_LIST_MAX, 최신순) */
...
typedef enum {
    PHOTO_RX_DELETE_ALL_STATE_NONE = 0,
    PHOTO_RX_DELETE_ALL_STATE_REQUESTED,
    PHOTO_RX_DELETE_ALL_STATE_RECEIVED,  /* 2026-08-21 — CAM이 접수하고 지울 개수를 알려온 상태.
                                            아직 실제 삭제 완료는 아님 — ACKED와 구분 */
    PHOTO_RX_DELETE_ALL_STATE_ACKED,
