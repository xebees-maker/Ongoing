 ...
 *
 * 2026-09-06 — 예전(2026-07-30) 커스텀 BSP 시도 때 만든 드라이버를 그대로 재사용
 * (`_Archive/Cntl_backup_20260730_224030/bsp_ws_4_3b/ch422g.c/.h`에서 가져옴, 그
 * BSP 자체는 폐기됐지만 이 드라이버는 그대로 유효). waveshare_rgb_lcd_port.c가
 * 쓰던 통짜 매직넘버(0x2C/0x2E/0x1E) 방식은 SD_CS까지 안전하게 같이 관리하기엔
 * 섀도우 상태가 이원화돼서 위험 — 이 드라이버로 통합해서 하나의 섀도우가 전체를
 * 관리하게 함(ch422g_set_io_raw()로 기존 매직넘버 시퀀스는 그대로 재현).
 *
 ...
 *        실제 레지스터 쓰기는 하지 않음(호출부가 첫 실제 시퀀스를 원하는 순서로 결정).
