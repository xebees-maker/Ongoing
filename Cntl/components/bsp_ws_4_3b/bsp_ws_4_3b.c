...
esp_err_t bsp_board_init(void)
{
    ...
    /* CH422G의 첫 실제 I2C 트랜잭션은 반드시 터치(GT911) 리셋 시퀀스여야 함 — ch422g_init()의
     * 주석에 실기 검증 근거가 있음("미리 다른 쓰기를 해두면 터치가 응답 안 함"). 예전엔 여기서
     * LCD_RST를 별도로 low->high 펄스 줬었는데, 그게 바로 그 규칙을 깨고 있었음(CH422G 첫
     * 쓰기가 LCD_RST가 돼버림) — 벤더 공식 데모(project_cntl_4_3b_bsp 메모리, 실기 검증됨)엔
     * 그런 별도 펄스가 아예 없고, LCD_RST는 터치 리셋 바이트(0x2C/0x2E, bsp_touch_reset_via_ch422g
     * 참고)에 이미 포함돼 있어서 그 시퀀스의 일부로 같이 풀림. 백라이트도 벤더처럼 터치 이후로
     * 옮김(원래도 여기 있었음, 순서만 터치 뒤로). */
...
static esp_err_t bsp_lcd_init(esp_lcd_panel_handle_t *out_panel)
{
    ...
    /* esp_lcd_panel_reset()은 호출 안 함 — 벤더 공식 데모도 안 씀(RGB 패널이라
     * disp_gpio_num=-1, reset()은 이 패널 타입에선 사실상 no-op). */
